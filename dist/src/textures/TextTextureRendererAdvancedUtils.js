 ...
 * Style helper
 */
export function createLineStyle(tags, baseFont, color) {
 ...
 */
export function layoutSpans(ctx, spans, lineStyle, wrapWidth, textIndent, maxLines, suffix, wordBreak, letterSpacing, allowTruncation) {
    // styling
    const { isStyled, baseStyle, updateStyle, getStyle } = lineStyle;
    const initialStyle = getStyle();
    ctx.font = initialStyle.font;
    let style = isStyled ? initialStyle : undefined;
    // cached metrics
...
export function trimWordStart(text, rtl) {
    if (rtl) {
 ...
 * Render text lines
 */
export function renderLines(ctx, lines, lineStyle, align, lineHeight, wrapWidth, indent) {
...
//# sourceMappingURL=TextTextureRendererAdvancedUtils.js.map