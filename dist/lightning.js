...
`;
  ...
  class Application extends Component {
    ...
    focusTopDownEvent(events, ...args) {
      ...
      for (let i = 0; i < n; i++) {
        const target = path[i];
        const events2 = isHorizontalDirection && target.rtl ? eventsRtl : eventsLtr;
        const event = target._getMostSpecificHandledMember(events2);
        if (event !== void 0) {
    ...
    _receiveKeydown(e) {
      ...
      if (keys) {
        for (let i = 0, n = keys.length; i < n; i++) {
          const key2 = keys[i];
          const hasTimer = this.__keypressTimers.has(key2);
          if (path[path.length - 1].longpress && hasTimer) {
            return;
          }
          if (!this.focusTopDownEvent([`_capture${key2}`, "_captureKey"], obj)) {
            this.focusBottomUpEvent([`_handle${key2}`, "_handleKey"], obj);
          }
        }
      } else {
    ...
    _receiveKeyup(e) {
      ...
      if (keys) {
        for (let i = 0, n = keys.length; i < n; i++) {
          const key2 = keys[i];
          if (!this.focusTopDownEvent([`_capture${key2}Release`, "_captureKeyRelease"], obj)) {
            this.focusBottomUpEvent([`_handle${key2}Release`, "_handleKeyRelease"], obj);
          }
        }
      } else {
